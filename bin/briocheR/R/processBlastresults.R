...
#' @param btop character string showing variants between qseq and sseq
#' @noRd
  ...
  tryCatch({
    ...
    calculateSNPpos <- function(qstart, qend, sstart, send, sstrand, qseq, sseq, target_bp) {
      ...
      if (k < 1L || k > q_len)  {
        ...
        # total aligned subject length (counts only non-gaps in sseq)
        s_len <- stringi::stri_count_regex(sseq, "[^-]")
        
        if (k > q_len) {
          # K is located after the end of the alignment
          # last aligned subject base
          last_pos <- if (is_plus) (s_first + s_len - 1L) else (s_first - s_len + 1L)
          overhang <- k - q_len                  
          if (is_plus) {
            return(paste0(">", last_pos + overhang))
          } else {
            return(paste0("<", last_pos - overhang))
          }
        } else {
          # ---- BEFORE qstart (k <= 0) ----
          # first aligned subject base is s_first
          overhang <- 1L - k                     
          if (is_plus) {
            return(paste0("<", s_first - overhang))
          } else {
            return(paste0(">", s_first + overhang))
          }
        }
    ...
    write.table(
      subset(
        data,
        SNP.Refpos != paste0(saccver, ":.-.") &
          SNP.Refpos != paste0(saccver, ":NA-NA") &
          !is.na(SNP.Refpos) &
          !grepl("[<>]", SNPpos)
      )$SNP.Refpos,
